...
static void continuous_phase_retrieval(double *s_out_f) {
    ...
    double lambda_sp = 5.0;    // Titanium walls
    ...
    /* Generate Spatial A and b for continuous evaluation without GF(q) butterfly hashing */
    ...
    for(int step=1; step<=15000; step++) {
        ...
        /* NEW: Integer Crystallization Wells — scaled by lr to stay within gradient budget */
        double lambda_quant = (step > 5000) ? lr * 0.3 : 0.0;
        ...
        for(int i=0;i<256;i++) {
            ...
            /* Apply quantization force scaled to lr so wells don't overpower gradient */
            if (lambda_quant > 0) {
                s0[i] -= lambda_quant * sin(s0[i] * 2.0 * M_PI);
                s1[i] -= lambda_quant * sin(s1[i] * 2.0 * M_PI);
            }
        ...
        if(step % 500 == 0 || (viol_sp == 0 && E_freq < 1.0)) {
            printf("    Step %5d: E_freq=%.4e E_sp=%.4e, Viol=%d/512  (lr=%.4f, Q=%.1f, best=%.4e)\n",
                   step, E_freq, E_sp, viol_sp, lr, Q_eff, best_E);
            fflush(stdout);
        }
    }

    /* Output best (minimum E_freq) spatial solution */
    printf("    [Phase2] Best E_freq achieved: %.4e (at some step before divergence)\n", best_E);
    for(int j=0;j<256;j++) {
        s_out_f[j]     = s0[j];
        s_out_f[256+j] = s1[j];
    }
