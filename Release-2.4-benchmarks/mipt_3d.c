...
/* Actively prune tiny amplitudes to maintain strict sparsity */
static void compress_register(QuhitEngine *eng, int reg_idx, double threshold)
{
    ...
    /* Renormalize the remaining surviving state */
    if (n2 > 1e-20) {
...
static void compress_all(Tns3dGrid *g)
{
    int N = g->Lx * g->Ly * g->Lz;
    for (int i = 0; i < N; i++) {
        compress_register(g->eng, g->site_reg[i], 1e-6); // Prune below 1e-6 magnitude squared
    }
}
                     ...
                     double *gz_re, double *gz_im)
{
    ...
    for (int step = 1; step <= MIPT_STEPS; step++) {
        ...
        /* Actively enforce sparsity to prevent exponential slowdown from SVD epsilon noise */
        compress_all(g);
