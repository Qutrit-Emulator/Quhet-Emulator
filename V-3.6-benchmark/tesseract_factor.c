...
/* #include "reality_source.h" -- not needed with MPS overlay */
#include "bigint.h"
#include "quhit_dyn_integrate.h"
#include "s6_exotic.h"
#include "mps_overlay.h"   /* Enhanced vesica fold SVD pipeline */
...
typedef struct {
    ...
    MpsChain *mps;              /* MPS overlay: 3 sites per tess, vesica fold SVD */
} TesseractArray;

static double MPS_DFT_RE[36], MPS_DFT_IM[36];
static double MPS_CZ_RE[36*36], MPS_CZ_IM[36*36];
static int vesica_gates_built = 0;

static void ensure_vesica_gates(void) {
    if (vesica_gates_built) return;
    mps_build_dft6(MPS_DFT_RE, MPS_DFT_IM);
    mps_build_cz(MPS_CZ_RE, MPS_CZ_IM);
...
static void tessarray_init(TesseractArray *arr, int n_active)
{
    ...
    /* Initialize MPS chain: 3 quhit sites per tesseract */
    int mps_len = n_active * 3;
    arr->mps = mps_init(mps_len);

    /* Put all MPS sites into |+⟩ superposition via DFT */
    mps_gate_1site_all(arr->mps, MPS_DFT_RE, MPS_DFT_IM);
                           ...
                           const BigInt *N, int loop_idx)
{
    ...
    /* ── Inter-Tesseract Entanglement via MPS Vesica Fold ──
     * Route all entanglement through the MPS overlay with
     * tsvd_vesica_truncated_sparse + 15-way omnidirectional sweep.
     * CZ bond gates between adjacent MPS sites replace the manual
     * phase-lock bonding — the vesica fold SVD handles the compression. */
    if (arr->mps) {
        /* CZ on all adjacent bonds within the MPS chain */
        for (int i = 0; i < arr->mps->L - 1; i += 2)
            mps_gate_bond(arr->mps, i, MPS_CZ_RE, MPS_CZ_IM);
        for (int i = 1; i < arr->mps->L - 1; i += 2)
            mps_gate_bond(arr->mps, i, MPS_CZ_RE, MPS_CZ_IM);
    }
                               ...
                               DynChain *dyn)
{
    ...
    /* Inter-tesseract entanglement via MPS vesica fold — active sites only */
    int active_len = dyn_chain_active_length(dyn);
    if (active_len < 2) return;

    if (arr->mps) {
        /* CZ bonds between active MPS sites (3 sites per tesseract) */
        int mps_start = dyn->active_start * 3;
        int mps_end   = (dyn->active_end + 1) * 3 - 1;
        for (int i = mps_start; i < mps_end; i += 2)
            if (i < arr->mps->L - 1)
                mps_gate_bond(arr->mps, i, MPS_CZ_RE, MPS_CZ_IM);
        for (int i = mps_start + 1; i < mps_end; i += 2)
            if (i < arr->mps->L - 1)
                mps_gate_bond(arr->mps, i, MPS_CZ_RE, MPS_CZ_IM);
    }
...
static void apply_qft_all(TesseractArray *arr)
{
    ...
    /* Vesica-fold-enhanced QFT pass: DFT + CZ sweep through MPS */
    if (arr->mps) {
        mps_gate_1site_all(arr->mps, MPS_DFT_RE, MPS_DFT_IM);
        for (int i = 0; i < arr->mps->L - 1; i += 2)
            mps_gate_bond(arr->mps, i, MPS_CZ_RE, MPS_CZ_IM);
