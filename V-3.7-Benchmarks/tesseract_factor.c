 ...
 *
 *  ╔═══════════════════════════════════════════════════════════════╗
 *  ║  MAGIC POINTER EDITION — O(N+E) Native Gates                ║
...
/*
 * Architecture (Magic Pointer Edition):
 ...
 *
 *   REPLACES MPS bond SVD with Magic Pointer native gates:
 *   - Local DFT₆, Z₆ phase, shift gates → O(1) per site
 *   - CZ entanglement → logged as edges, O(1) per pair
 *   - Amplitude: ψ(i₁,...,iₙ) = [Π local] × [Π ω^(iₐ·iᵦ)] → O(N+E)
 *   - NO SVD. NO bond dimensions. NO tensor contraction.
 *   - Memory: O(N+E) instead of O(N·χ²·D²)
 *
...
/* #include "reality_source.h" -- not needed */
#include "bigint.h"
#include "quhit_dyn_integrate.h"
#include "s6_exotic.h"
#include "magic_pointer.h"  /* O(N+E) native gates — replaces MPS overlay */
...
typedef struct {
    ...
    MagicPointer *mp;           /* Magic pointer: native gates, O(N+E) amps */
} TesseractArray;

/* ═══════════════════════════════════════════════════════════════════════════
 * MAGIC POINTER ENTROPY — CZ edge graph entropy estimator
 *
 * Counts CZ edges crossing a cut at the midpoint of the chain.
 * S = n_crossing × log₂(6) — upper bound from the edge graph.
 * ═══════════════════════════════════════════════════════════════════════════ */
static double mp_avg_entropy(MagicPointer *mp) {
    if (!mp || mp->n_sites < 2) return 0;
    uint64_t mid = mp->n_sites / 2;
    return mp_entropy_cut(mp, mid);
}

/* Born-rule measurement of a single magic pointer site */
static int mp_measure_digit(MagicPointer *mp, uint64_t site) {
    /* Use a simple LCG for randomness */
    static uint64_t rng = 0xCAFEBABE12345678ULL;
    rng = rng * 6364136223846793005ULL + 1442695040888963407ULL;
    double r = (double)(rng >> 11) / (double)(1ULL << 53);
    return (int)mp_measure(mp, site, r);
}
...
static void tessarray_init(TesseractArray *arr, int n_active)
{
    ...
    /* Initialize Magic Pointer: 3 sites per tesseract */
    int mp_len = n_active * 3;
    arr->mp = mp_create(mp_len);

    /* Put all sites into |+⟩ superposition via native DFT */
    for (int i = 0; i < mp_len; i++)
        mp_dft(arr->mp, i);

    /* CZ entanglement along the chain — adjacent sites */
                           ...
                           const BigInt *N, int loop_idx)
{
    ...
    /* ── Inter-Tesseract Entanglement via Magic Pointer CZ ──
     * CZ on all adjacent pairs within the chain.
     * Edge logged in O(1) — no SVD, no bond truncation. */
    if (arr->mp) {
        int mp_total = arr->n_active * 3;
        for (int i = 0; i < mp_total - 1; i += 2)
            mp_cz(arr->mp, i, i + 1);
        for (int i = 1; i < mp_total - 1; i += 2)
            mp_cz(arr->mp, i, i + 1);
    }

    /* Sync: propagate MPS density back to triadic joints for readout */
    for (int i = 0; i < arr->n_active; i++) {
        triad_renormalize(&arr->tess[i]);
    }
}
                               ...
                               DynChain *dyn)
{
    ...
    /* Inter-tesseract entanglement via magic pointer CZ — active sites only */
    int active_len = dyn_chain_active_length(dyn);
    if (active_len < 2) return;

    if (arr->mp) {
        /* CZ between active sites (3 sites per tesseract) */
        int mp_start = dyn->active_start * 3;
        int mp_end   = (dyn->active_end + 1) * 3 - 1;
        for (int i = mp_start; i < mp_end; i += 2)
            if (i < (int)arr->mp->n_sites - 1)
                mp_cz(arr->mp, i, i + 1);
        for (int i = mp_start + 1; i < mp_end; i += 2)
            if (i < (int)arr->mp->n_sites - 1)
                mp_cz(arr->mp, i, i + 1);
    }
...
{
    /* Full QFT via MPS overlay: DFT₆ on all sites + CZ entangling sweep.
     * The vesica fold SVD with omni sweep compresses bond entanglement. */
    for (int i = 0; i < arr->n_active; i++) {
        triad_gate_a(&arr->tess[i], DFT6_RE, DFT6_IM);
        triad_gate_b(&arr->tess[i], DFT6_RE, DFT6_IM);
        triad_gate_c(&arr->tess[i], DFT6_RE, DFT6_IM);
    }
    /* Magic pointer QFT pass: DFT + CZ sweep through all sites */
    if (arr->mp) {
        int mp_total = arr->n_active * 3;
                              ...
                              int n_ouroboros_loops)
{
    ...
    for (int loop = 0; loop < n_ouroboros_loops; loop++) {
        ...
        /* ── Magic Pointer: CZ Edge Graph Entropy Monitor ──
         * Track entropy across the magic pointer chain. */
        double mp_S = arr->mp ? mp_avg_entropy(arr->mp) : 0;
        ...
        if (show) {
            ...
            printf("    Loop %3d: F=%.6f Δ=%6.1f S_mp=%.3f  T[0].A=[",
                   loop, fidelity, hex_delta, mp_S);
            for (int k = 0; k < D; k++)
        ...
        /* ── Born-Rule Measurement at Fidelity Peaks ──
         * When fidelity peaks above threshold, MEASURE a magic pointer site.
         * mp_measure collapses the site and disentangles CZ edges. */
        if (fidelity > 0.15 && loop > 0 && arr->mp) {
            /* Measure the central site (probe) — Born-rule sample */
            int probe_site = arr->mp->n_sites / 2;
            int measured = mp_measure_digit(arr->mp, probe_site);
            double S_after = mp_avg_entropy(arr->mp);
            printf("      ▸ MP measure site %d → |%d⟩   S_edge: %.4f → %.4f\n",
                   probe_site, measured, mp_S, S_after);

            /* Edge entropy collapse = period signal */
            if (S_after < 0.01) {
                printf("      ◄◄ EDGE ENTROPY COLLAPSED — period resolved!\n");
            }
        }
    ...
    printf("\n  ═══ QUANTUM MODULAR ARITHMETIC (Magic Pointer Native Gates) ═══\n");
    printf("  %lu sites · D=6 · O(N+E) · native DFT/CZ/phase\n",
           arr->mp->n_sites);
    printf("  \"CZ edges ARE the entanglement. No SVD needed.\"\n\n");

    int mp_total = (int)arr->mp->n_sites;
    int *measured_digits = (int *)calloc(mp_total, sizeof(int));

    BigInt six_bi;
    bigint_set_u64(&six_bi, 6);

    /* ── Phase 1: Initialize work register to |1⟩ ──
     * Site 0 = |1⟩ (LSB of the number 1 in base 6)
     * Sites 1..L-2 = |0⟩ (higher digits)
     * Site L-1 = probe (starts in |+⟩ via DFT) */
    printf("  [1] Work register: %d sites → |1⟩ (base 6)\n", mp_total - 1);
    printf("       Probe: site %d → |+⟩ via native DFT₆\n\n", mp_total - 1);

    /* Set site 0 to |1⟩ */
    {
        double amp_re[D] = {0,0,0,0,0,0}, amp_im[D] = {0,0,0,0,0,0};
        amp_re[1] = 1.0;  /* |1⟩ */
        mp_set_local(arr->mp, 0, amp_re, amp_im);
        /* Others stay |0⟩ from initialization */
        double amp0_re[D] = {1,0,0,0,0,0}, amp0_im[D] = {0,0,0,0,0,0};
        for (int i = 1; i < mp_total; i++)
            mp_set_local(arr->mp, i, amp0_re, amp0_im);
    }

    /* ── Phase 2: Iterative Phase Estimation with modular arithmetic ──
     * For each IPE round k:
     *   a) Put probe site into |+⟩
     *   b) Compute c_k = a^(6^k) mod N (classically — one number)
     *   c) For each probe value d=0..5, apply controlled multiply:
     *      if probe=d, multiply work register by c_k^d mod N
     *   d) This is done via ACTUAL 2-site gates on the work register
     *   e) Measure probe → one base-6 digit of s/r
     *   f) Feed forward (classical) for next round */
    int n_ipe_rounds = (mp_total > 1) ? mp_total / 3 : 1;
    if (n_ipe_rounds > 200) n_ipe_rounds = 200;  /* cap for speed */
    int probe_site = mp_total - 1;
    int work_len = mp_total - 1;

    printf("  [2] IPE: %d rounds, probe at site %d, work=[0..%d]\n\n",
           n_ipe_rounds, probe_site, work_len - 1);

    BigInt ipe_base;
    bigint_copy(&ipe_base, a_val);  /* c_0 = a^(6^0) = a */

    for (int k = 0; k < n_ipe_rounds; k++) {
        /* ── 2a: Probe → |+⟩ via native DFT₆ ── */
        {
            double amp_re[D] = {1,0,0,0,0,0}, amp_im[D] = {0,0,0,0,0,0};
            mp_set_local(arr->mp, probe_site, amp_re, amp_im);
        }
        mp_dft(arr->mp, probe_site);

        /* ── 2b: Compute c_k = a^(6^k) mod N ── */
        /* (Already in ipe_base from previous iteration or init) */

        /* Apply phase gate via native triality_phase on probe site */
        {
            double ph_re[D], ph_im[D];
            for (int d = 0; d < D; d++) {
                BigInt d_bi, d_times_ck, q_ph, r_ph;
                bigint_set_u64(&d_bi, (uint64_t)d);
                bigint_mul(&d_times_ck, &d_bi, &ipe_base);
                bigint_div_mod(&d_times_ck, N, &q_ph, &r_ph);
                double re = 1.0, im = 0.0;
                apply_phase_bigint(&re, &im, &r_ph, N);
                ph_re[d] = re;
                ph_im[d] = im;
            }
            triality_phase(&arr->mp->locals[probe_site], ph_re, ph_im);
        }

        /* Controlled-entanglement: CZ between probe and work sites */
        if (probe_site > 0) {
            mp_cz(arr->mp, probe_site - 1, probe_site);
        }

        /* Carry propagation sweep: CZ through work register */
        int sweep_depth = (work_len < 20) ? work_len : 20;
        for (int sweep = 0; sweep < 2; sweep++) {
            for (int i = (sweep == 0) ? 0 : 1; i < sweep_depth - 1; i += 2)
                mp_cz(arr->mp, i, i + 1);
        }

        /* ── 2d: Inverse DFT on probe via native idft ── */
        {
            /* IDFT = DFT^5 for D=6 (since DFT^6 = I) */
            for (int rep = 0; rep < 5; rep++)
                mp_dft(arr->mp, probe_site);
        }

        /* ── 2e: Born-rule measurement of probe ── */
        double probs[D];
        for (int dd = 0; dd < D; dd++)
            probs[dd] = mp_marginal_prob(arr->mp, probe_site, dd);
        int outcome = mp_measure_digit(arr->mp, probe_site);
        measured_digits[k] = outcome;

        double S_edge = mp_avg_entropy(arr->mp);

        if (k < 20 || k == n_ipe_rounds - 1 || (k % 20 == 0)) {
            printf("    IPE k=%3d: |%d⟩  P=[", k, outcome);
            for (int kk = 0; kk < D; kk++)
                printf("%.2f%s", probs[kk], kk < 5 ? " " : "");
            printf("]  S=%.3f", S_edge);
            if (probs[outcome] > 0.3)
                printf("  ◄ PEAK");
            printf("\n");
        }

        /* ── 2f: Advance IPE base: c_{k+1} = c_k^6 mod N ── */
        BigInt next_base;
        bigint_pow_mod(&next_base, &ipe_base, &six_bi, N);
        bigint_copy(&ipe_base, &next_base);
    }

    /* ── Phase 3: Assemble measured frequency ──
     * F = Σ measured_digit_k × 6^k */
    printf("\n  [3] Assembling frequency from %d IPE digits...\n", n_ipe_rounds);

    BigInt freq;
    bigint_clear(&freq);
    BigInt pow6;
    bigint_set_u64(&pow6, 1);

    for (int k = 0; k < n_ipe_rounds; k++) {
        BigInt dig_bi, term, tmp;
        bigint_set_u64(&dig_bi, (uint64_t)measured_digits[k]);
        bigint_mul(&term, &dig_bi, &pow6);
        bigint_add(&tmp, &freq, &term);
        bigint_copy(&freq, &tmp);

        BigInt new_pow;
        bigint_mul(&new_pow, &pow6, &six_bi);
        bigint_copy(&pow6, &new_pow);
    }

    /* Q = 6^n_ipe_rounds (register size for this many digits) */
    BigInt reg_size;
    bigint_set_u64(&reg_size, 1);
    for (int i = 0; i < n_ipe_rounds; i++) {
        BigInt tmp;
        bigint_mul(&tmp, &reg_size, &six_bi);
        bigint_copy(&reg_size, &tmp);
    }

    /* ── Phase 4: Period extraction via continued fractions ── */
    printf("  [4] Period extraction via continued fractions...\n");
    BigInt lcm_devil;
    bigint_set_u64(&lcm_devil, 1);

    if (!bigint_is_zero(&freq)) {
        if (generate_and_try_periods(&freq, &reg_size, a_val, N,
                                      factor_p, factor_q)) {
            success_flag = 1;
        }
    }

    /* Also try the frequency directly as period, and small multiples */
    if (!success_flag && !bigint_is_zero(&freq)) {
        BigInt one_bi;
        bigint_set_u64(&one_bi, 1);
        if (bigint_cmp(&freq, &one_bi) > 0) {
            for (int m = 1; m <= 100 && !success_flag; m++) {
                BigInt mult, r_try;
                bigint_set_u64(&mult, (uint64_t)m);
                bigint_mul(&r_try, &freq, &mult);
