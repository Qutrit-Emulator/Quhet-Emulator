...
typedef struct {
    ...
    int        capacity;
    int        N;
    uint64_t   explicit_mask; /* bit i=1 → quhit i has explicit channels */
} ThreeCh_Register;
 ...
 *
 * Stage 1: Hadamard on bit qi of b (always)
 * Stage 2: Twiddle ω₆^(c_i · b_i) (skip if uniform: c_i=0)
 * Stage 3: DFT₃ on c_i (skip if uniform: only c_i=0 populated,
 *          DFT₃ output is uniform → stays at c_i=0 with same amp)
 *
 * For EXPLICIT quhits: full 3-stage factored DFT₆.
 * For UNIFORM quhits: just Hadamard. 3^N savings.
 */
static inline void tcr_apply_dft6(ThreeCh_Register *r, int qi) {
    uint64_t mask = 1ULL << qi;
    int is_explicit = (r->explicit_mask >> qi) & 1;
    ...
    if (!is_explicit) {
        /* Uniform: c_i=0 for all entries.
         * Twiddle: ω₆^0 = 1, no-op.
         * DFT₃: input (a, 0, 0) → output (a/√3, a/√3, a/√3).
         * All 3 outputs equal → stays uniform at c_i=0.
         * No amplitude change needed (normalization absorbed in Born probs). */
        tcr_trim(r, 1e-15);
        return;
    }
...
/* ═══════════════ Make quhit qi explicit ═══════════════
 *
 * Expand uniform c_i=0 entries into 3 copies (c_i=0,1,2)
 * each with amplitude scaled by 1/√3 (DFT₃ normalization).
 */
static inline void tcr_make_explicit(ThreeCh_Register *r, int qi) {
    if ((r->explicit_mask >> qi) & 1) return; /* already explicit */
    ...
    for (int i = 0; i < old_count; i++) {
        for (int c = 0; c < 3; c++) {
            uint64_t cv = tcr_set_c(old[i].cvec, qi, c);
            tcr_add(r, cv, old[i].b,
                    old[i].re * TCR_N3, old[i].im * TCR_N3);
        }
    }
    free(old);
    r->explicit_mask |= (1ULL << qi);
}

/* ═══════════════ CZ Gate ═══════════════ */

static inline void tcr_apply_cz(ThreeCh_Register *r, int qi, int qj) {
    /* Expand both quhits to explicit channels */
    tcr_make_explicit(r, qi);
    tcr_make_explicit(r, qj);

    /* Apply diagonal CZ phase */
...
static inline void tcr_born_probs(const ThreeCh_Register *r,
    ...
    for (int e = 0; e < r->count; e++) {
        ...
        if (is_explicit) {
            ...
            /* Uniform: distribute equally across 3 channels */
            for (int s = 0; s < 3; s++)
                probs[3 * p + s] += mag2 / 3.0;
        }
    }
}
...
static inline double tcr_correlation(const ThreeCh_Register *r,
                                     int qi, int qj) {
    double corr = 0;
    int ex_i = (r->explicit_mask >> qi) & 1;
    int ex_j = (r->explicit_mask >> qj) & 1;
    ...
    for (int e = 0; e < r->count; e++) {
        ...
        if (ex_i && ex_j) {
            int ki = 3*bi + tcr_get_c(r->entries[e].cvec, qi);
            int kj = 3*bj + tcr_get_c(r->entries[e].cvec, qj);
            corr += (double)(ki * kj) * mag2;
        } else if (ex_i) {
            int ki = 3*bi + tcr_get_c(r->entries[e].cvec, qi);
            /* Average kj over 3 channels: E[3bj+s] = 3bj+1 */
            corr += (double)ki * (3.0*bj + 1.0) * mag2;
        } else if (ex_j) {
            int kj = 3*bj + tcr_get_c(r->entries[e].cvec, qj);
            corr += (3.0*bi + 1.0) * (double)kj * mag2;
        } else {
            /* Both uniform: E[k_i*k_j] = E[3bi+s]*E[3bj+s'] */
            /* E[3b+s] for uniform s∈{0,1,2}: 3b + 1 */
            corr += (3.0*bi + 1.0) * (3.0*bj + 1.0) * mag2;
        }
    }
    return corr;
}
...
static inline void tcr_print(const ThreeCh_Register *r, const char *label) {
    int n_exp = __builtin_popcountll(r->explicit_mask);
    int n_uni = r->N - n_exp;
    printf("  %s: N=%d, %d entries (%d explicit, %d uniform quhits)\n",
           label, r->N, r->count, n_exp, n_uni);
    printf("    prob=%.6f, compression: %.0f → %d (%.1f×)\n",
           tcr_total_prob(r), pow(6.0, r->N), r->count,
           pow(6.0, r->N) / r->count);
}
